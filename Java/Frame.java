...
public class Frame {
     ...
     */
    public static String draw(String[] frameArray, boolean openFrame) {
        ...
        for (String frame : frames) {
            for (String line : frame.split("\n")) {
                output.append(border).append(line);
                int l = line.length();
                while (l < WIDTH) {
                    output.append(" ");
                    l++;
    ...
    public static String draw(String[] frameArray) {
        return draw(frameArray, false);
    }
     ...
     */
    public static String one(String input, String title, boolean formatInput) {
        String[] out = new String[2];
        // TODO: check to see if title needs formatting
        out[0] = title;
    ...
    public static String format(String input, String alignment, char spacer) {
        ...
        if (input.equalsIgnoreCase(String.valueOf(spacer))) {
            ...
            return String.valueOf(spacer).repeat(WIDTH);
     ...
     * This method creates a formatted frame from a string.
     *
     * @param leftString   left side of the split
     * @param rightString  right side of the split
     * @param spacer character used for string padding
     */
    public static String split(String leftString, String rightString, char spacer) {
        // add spacing
        if (leftString.charAt(0) != ' ') {
            leftString = " " + leftString;
        }
        if (rightString.charAt(rightString.length() - 1) != ' ') {
            rightString = rightString + " ";
        }
        // check if string is valid
        if (leftString.length() + rightString.length() + 1 > WIDTH) {
            return leftString + spacer + rightString;
        }
        // build string
        StringBuilder output = new StringBuilder(leftString);
        while (output.length() <= (WIDTH - rightString.length() - 1)) {
            output.append(spacer);
        }
        return output.append(rightString).toString();
    }

    // overload - 2 inputs (String, String)
    public static String split(String leftString, String rightString) {
        return split(leftString, rightString, '.');
    }
